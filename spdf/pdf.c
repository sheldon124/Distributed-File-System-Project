...
//Utility Function to check if a file existss
...
void downloadHandler(const char *path, int client){
    ...
    if(!checkIfFileExists1(fullPath)){ 
        ...
        // Wat for acknowledgment from te main server
        char ackBuffer[9];
...
void tarHandler(int client){
    ...
    if(!pdfFilesExist){
        ...
        // Wait foir acknowledgment from the masin server
        char ackBuffer[9];
        if (recv(client, ackBuffer, sizeof(ackBuffer) - 1, 0) < 0) {
            printf("Main server acknowledgment failed\n");
            close(tarfd);
...
int prcclient(char* inputcommand, int client) {
    ...
    else if(strcmp(cmd, "rmfile") == 0) { // process rmfile command
